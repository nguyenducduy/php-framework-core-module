...
use Shirou\UserException as Exception;
...
class BaseCommand implements ICommand
{
    ...
    public function printParameters($action)
    {
        ...
        if (empty($this->_actions[$action])) {
            throw new Exception("Action '$action' not found in this command.");
        }
    ...
    protected function _parseParameters()
    {
        ...
        for ($i = 1; $i < $argumentsCount; $i++) {
            ...
            if (preg_match('#^([\-]{1,2})([a-zA-Z0-9][a-zA-Z0-9\-]*)(=(.*)){0,1}$#', $argv, $matches)) {
                if (empty($matches[2])) {
                    throw new Exception("Invalid script parameter '$argv'.");
                }
                ...
                switch ($annotation->getName()) {
                    ...
                    case 'CommandName':
                        ...
                        if (!isset($arguments[0]) || !is_array($arguments[0])) {
                            throw new Exception('Command name must be an array of available names.');
                        }
                    ...
                    case 'CommandDescription':
                        ...
                        if (!isset($arguments[0]) || !is_string($arguments[0])) {
                            throw new Exception('Command description must be a string.');
                        }
